...
<h3 align="center">
A cli to browse and watch anime (alone AND with friends). This tool supports multiple providers including <a href="https://allmanga.to/">AllAnime</a>, <a href="https://animepahe.si/">AnimePahe</a>, <a href="https://hianime.to/">HiAnime</a>, Netflix Mirror, JioHotstar Mirror, and PrimeVideo Mirror.
</h3>
...
ani-cli supports multiple streaming providers:

| Provider | Flag | Content Type | Features |
|----------|------|--------------|----------|
| **AllAnime** | `--allanime` (default) | Anime | Sub/Dub, Multiple qualities |
| **AnimePahe** | `--animepahe` | Anime | Sub/Dub, Multiple qualities (360p-1080p) |
| **HiAnime** | `--hianime` | Anime | Sub/Dub toggle |
| **Netflix Mirror** | `--netflix` | Movies/Series | Multiple audio tracks, Subtitles |
| **JioHotstar Mirror** | `--jiohotstar` | Movies/Series | Multiple audio tracks, Subtitles |
| **PrimeVideo Mirror** | `--primevideo` | Movies/Series | Multiple audio tracks, Subtitles |
| **Global Search** | `--global` | All | Search across all providers at once |

**Note:** AnimePahe requires Python 3 with Playwright installed for DDoS-Guard bypass. Install with:
```sh
...
- ani-skip (optional)
- patch - Self updating
- jq - JSON parsing (for some providers)
- python3 + playwright (optional, for AnimePahe DDoS-Guard bypass)
...
* Can I change subtitle language or turn them off? - For Netflix/JioHotstar/PrimeVideo, yes - use the `change_subtitle` menu option during playback. For anime providers, subtitles are usually baked into the video.
* Can I watch dub? - Yes, use `--dub`. For HiAnime and AnimePahe, you can also toggle between sub/dub using the `change_sub_dub` menu option during playback.
* Can I change dub language? - For Netflix/JioHotstar/PrimeVideo, yes - use the `change_audio` menu option. For anime providers, only Japanese (sub) and English (dub) are available.
* Can I change media source/provider? - Yes! Use `--netflix`, `--animepahe`, `--hianime`, `--allanime`, `--jiohotstar`, `--primevideo`, or `--global` for global search.
* Can I use vlc? - Yes, use `--vlc` or `export ANI_CLI_PLAYER=vlc`.
* Can I adjust resolution? - Yes, use `-q resolution`, for example `ani-cli -q 1080`. For AnimePahe, you'll get a quality selection menu with all available options.
...
* What is Global Search? - Use `--global` to search across all providers simultaneously and pick results from any source.
* AnimePahe shows DDoS-Guard error? - Install Playwright: `pip install playwright && playwright install chromium`

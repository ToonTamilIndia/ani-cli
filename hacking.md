# Hacking ani-cli
Ani-cli is set up to support multiple platforms - including AllAnime, AnimePahe, HiAnime, Netflix Mirror, JioHotstar Mirror, and PrimeVideo Mirror. Each provider has its own set of functions for search, episode listing, and video extraction.

This guide helps you understand how to add new providers or modify existing ones.

## Current Provider Architecture

The script uses a provider-based architecture where each provider implements these core functions:

| Function | Purpose | Example |
|----------|---------|---------|
| `{provider}_search` | Search for anime/content | `animepahe_search`, `netflix_search` |
| `{provider}_episodes_list` | Get list of episodes | `animepahe_episodes_list` |
| `{provider}_get_episode_url` | Extract video URL | `animepahe_get_episode_url` |

Providers are selected via CLI flags (`--animepahe`, `--netflix`, etc.) or the interactive provider menu.

## Prerequisites
Here's the list of skills you'll need and the guide will take for granted:
...
1. **Define base URLs** at the top of the script:
   ```sh
   myprovider_base="https://example.com"
   ```

2. **Implement search function** (`myprovider_search`):
   - Input: search query
   - Output: `id\ttitle` format (tab-separated)

3. **Implement episodes list function** (`myprovider_episodes_list`):
   - Input: series ID
   - Output: episode numbers, one per line

4. **Implement episode URL function** (`myprovider_get_episode_url`):
   - Input: ID, episode number
   - Sets: `$episode` (video URL), `$refr_flag` (referrer if needed)

5. **Add CLI flag** in the argument parser:
   ```sh
   --myprovider) provider="myprovider" ;;
   ```

6. **Add to provider list** in `select_provider` function

7. **Add to `get_episode_url`** case statement

8. **Add to global search** (optional) in `global_search` function

### Handling DDoS Protection

Some sites use DDoS protection (Cloudflare, DDoS-Guard). See the `animepahe_bypass` function for an example using Playwright to solve JS challenges.
